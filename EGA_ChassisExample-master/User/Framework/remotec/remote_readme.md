...
- **搭配ia10b接收机时，即使遥控器关控，接收机也会持续往C板发送数据，内容是断连前发送的最后一帧数据，所以无法通过检测一定时间内未接收到数据来判断遥控器断连，这很致命!**
## 代码说明
- 在remotec_fs16中，我用了一个类来存储遥控器的数据，既方便又简洁，且安全性较高
- 除了各种变量外，我还存储了各种变量的last，这是用来比较两次数据是否有差别，若无差别，则认为遥控器闲置，若闲置时长过长，则认为遥控器掉线，从而将online设置为false。随后在底盘loop中通过getter主动获得遥控器是否在线来决定是否给底盘断电
- 此外，我还将switch3(第三个开关)设置为安全开关，当该开关在最上面(开机时强制要求的档位)时，则主动将rc_online设置为false，从而能快速掐断底盘电源
  - 由此可见，这个online并不真正代表遥控器是否在线，而是我们虚拟出来的一个数据。虽然现在的判断确实在某些时候(如底盘小陀螺但摇杆没拨时)会误将online设置为false导致底盘断电，但这也是个折中的选择，毕竟没有任何有效方法来完全判断遥控器是否关机()
- 综上
