...
void cRemote::UpdateRCTarget(float* rc_target, int16_t* mode) const
{
    rc_target[0] = -static_cast<float>(rc_left_vertical+21) / (7.84f * 4);
    rc_target[1] = static_cast<float>(rc_left_horizontal+4) / (7.84f * 4);
    rc_target[2] = -static_cast<float>(rc_right_horizontal+5);
    *mode = rc_switch_D;
